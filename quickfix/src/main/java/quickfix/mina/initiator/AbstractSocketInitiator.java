...
public abstract class AbstractSocketInitiator extends SessionConnector implements Initiator {
    ...
    private IoSessionInitiator createInitiator(final Session session) throws ConfigError, FieldConvertError {
        ...
        SocketAddress localAddress = getLocalAddress(getSettings(), sessionID);
        ...
        final String keyStorePassword = SSLSupport.getKeystorePasswd(getSettings(), sessionID);
        final String strEnableProtocole = SSLSupport.getEnableProtocole(getSettings(), sessionID);
        final String[] enableProtocole = strEnableProtocole != null
                ? strEnableProtocole.split(",")
                : null;
        final String strCipherSuites = SSLSupport.getCipherSuite(getSettings(), sessionID);
        final String[] cipherSuites = strCipherSuites != null ? strCipherSuites.split(",") : null;
        ...
        final IoSessionInitiator ioSessionInitiator = new IoSessionInitiator(session,
                ...
                getScheduledExecutorService(), networkingOptions, getEventHandlingStrategy(),
                getIoFilterChainBuilder(), sslEnabled, keyStoreName, keyStorePassword,
                enableProtocole, cipherSuites);

        this.initiators.add(ioSessionInitiator);
        
        return ioSessionInitiator;
    }
    ...
    private void createSessions() throws ConfigError, FieldConvertError {
        ...
        for (final Iterator<SessionID> i = settings.sectionIterator(); i.hasNext();) {
            ...
            if (isInitiatorSession(sessionID)) {
                try {
                    if (!settings.isSetting(sessionID, SETTING_INACTIVE_SESSION)
                            || !settings.getBool(sessionID, SETTING_INACTIVE_SESSION)) {
    ...
    public void createDynamicSession(SessionID sessionID) throws ConfigError {
        ...
        try {
            ...
            IoSessionInitiator initiator = createInitiator(session);
    ...
    protected abstract void registerSession(Session session);
}
